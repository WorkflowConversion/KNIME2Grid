...
#   - name of the input port - it is expected that the input port refers to an
      archive containing files using the following naming schema:
        N_[port_name], where N is a zero-based index
    - name of the output port - this is a gUSE "generator" port, so files will be
      renamed to use the following schema:
        [port_name]_N, where N is a zero-based index 
