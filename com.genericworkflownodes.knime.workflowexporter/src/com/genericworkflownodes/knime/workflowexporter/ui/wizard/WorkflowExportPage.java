...
public class WorkflowExportPage extends WizardPage {
	...
	public WorkflowExportPage(final Collection<KnimeWorkflowExporter> exporters) {
		super("com.genericworkflownodes.knime.ui.wizard.WorkflowExportPage",
				"Select the destination format", ImageRepository
	...
	public KnimeWorkflowExporter getSelectedExporter() {
		if (selectedIndex == -1) {
			throw new IllegalStateException(
					"No exporter has been selected yet!");
		}
	...
	public boolean isPageComplete() {
		return selectedIndex != -1
				&& StringUtils.isNotBlank(destinationFilePath);
	}
	...
	public void createControl(final Composite parent) {
		...
		final Label formatLabel = new Label(selectFormatGroup, SWT.HORIZONTAL
				| SWT.LEFT);
		formatLabel.setText("Select an export format:");
		// formatLabel.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		final Combo formatCombo = new Combo(selectFormatGroup, SWT.PUSH
				| SWT.DROP_DOWN);
		...
		selectDestinationGroup.setLayoutData(new GridData(
				GridData.FILL_HORIZONTAL));

		final Label destinationLabel = new Label(selectDestinationGroup,
				SWT.NULL);
		destinationLabel.setText("Select an export destination:");

		final Text destinationText = new Text(selectDestinationGroup,
				SWT.BORDER | SWT.SINGLE);
		destinationText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		final Button destinationButton = new Button(selectDestinationGroup,
				SWT.PUSH);
			...
			public void modifyText(final ModifyEvent e) {
				if (e.getSource() == formatCombo) {
					...
					if (selectedIndex > -1) {
						...
						final KnimeWorkflowExporter selectedExporter = exporters
								.get(selectedIndex);
						final Image scaledPic = ImageDescriptor
								.createFromImageData(
										selectedExporter.getImageDescriptor()
												.getImageData()
												.scaledTo(150, 150))
								.createImage();
						image.setImage(scaledPic);
						description.setText(selectedExporter
								.getLongDescription());
					}
				}
				updatePageComplete();
		...
		destinationButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(final SelectionEvent e) {
				if (e.getSource() == destinationButton) {
					...
					final KnimeWorkflowExporter selectedExporter = exporters
							.get(selectedIndex);
					final Collection<ExtensionFilter> extensionFilters = selectedExporter
							.getExtensionFilters();
					final String[] filterExtensions = new String[extensionFilters
							.size()];
					final String[] filterNames = new String[extensionFilters
							.size()];
					int i = 0;
					for (final Iterator<ExtensionFilter> it = extensionFilters
