...
import com.genericworkflownodes.knime.workflowexporter.model.Job.JobBuilder;
...
public class InternalModelConverter {
	...
	private final static NodeLogger LOGGER = NodeLogger
			.getLogger(InternalModelConverter.class);
	...
	public Workflow convert() throws Exception {
		final WorkflowRootEditPart workflowRootEditPart = (WorkflowRootEditPart) editor
		...
		for (final EditPart ep : children) {
			if (ep instanceof NodeContainerEditPart) {
				...
				final NodeContainer nc = ((NodeContainerEditPart) ep)
						.getNodeContainer();
				...
				if (nc instanceof SingleNodeContainer) {
					...
					final Rectangle nodeBounds = ((NodeContainerEditPart) ep)
							.getFigure().getBounds();
					...
					if (((SingleNodeContainer) nc).getNodeModel() instanceof GenericKnimeNodeModel) {
						final GenericKnimeNodeModel nodeModel = (GenericKnimeNodeModel) ((SingleNodeContainer) nc)
								.getNodeModel();
						description = nodeModel.getNodeConfiguration()
								.getDescription();
					}
					...
					jobBuilder.setX(nodeBounds.x).setY(nodeBounds.y)
							.setId(id.toString()).setName(name)
							.setNrInputs(nc.getNrInPorts() - 1)
							.setNrOutputs(nc.getNrOutPorts() - 1)
					...
					if (nm instanceof GenericKnimeNodeModel) {
						...
						for (final String key : gknModel.getNodeConfiguration()
								.getParameterKeys()) {
							final Object paramValue = gknModel
									.getNodeConfiguration().getParameter(key)
									.getValue();
							final String strValue = paramValue == null ? ""
									: paramValue.toString();
		...
		final WorkflowManager workflowManager = workflowRootEditPart
				.getWorkflowManager();
		...
		for (final ConnectionContainer connectionContainer : workflowManager
			...
			final NodeContainer sourceContainer = workflowManager
					.getNodeContainer(connectionContainer.getSource());
			...
			if (sourceContainer instanceof SingleNodeContainer) {
				...
				if (snc.getNodeModel() instanceof GenericKnimeNodeModel) {
					final GenericKnimeNodeModel sourceModel = (GenericKnimeNodeModel) snc
							.getNodeModel();
					// get the ports into arrays
					// the data is the one coming from the source port
//					outputData = inputData = "output_"
//							+ sourceModel.getNodeConfiguration()
//									.getOutputPorts()[sourcePortNr].getName()
//							+ '.'
//							+ sourceModel.getOutputType(sourcePortNr)
//									.getExtension();
//					outputName = sourceModel.getNodeConfiguration()
//							.getOutputPorts()[sourcePortNr].getName();

				} else if (snc.getNodeModel() instanceof MimeFileImporterNodeModel) {
					final MimeFileImporterNodeModel nodeModel = (MimeFileImporterNodeModel) snc
							.getNodeModel();
					//outputData = inputData = nodeModel.getFileName();
				}
			}
			// the input name is obtained from the target port
			final NodeContainer targetContainer = workflowManager
					.getNodeContainer(connectionContainer.getDest());
			if (targetContainer instanceof SingleNodeContainer) {
				final SingleNodeContainer snc = (SingleNodeContainer) targetContainer;
				if (snc.getNodeModel() instanceof GenericKnimeNodeModel) {
					final GenericKnimeNodeModel targetModel = (GenericKnimeNodeModel) snc
							.getNodeModel();
//					inputName = targetModel.getNodeConfiguration()
//							.getInputPorts()[targetPortNr].getName();
				}
			}
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Added connection: " + sourceJob + "->"
						+ targetJob);
			}
		}
			...
			if (ep instanceof NodeContainerEditPart) {
				...
				for (final Object o : ep.getChildren()) {
					if (o instanceof NodeInPortEditPart) {
						...
						final Output output = job
								.getOutput(outPort.getIndex() - 1);
	...
	private void validateWorkflow(final Workflow workflow) {
		...
		for (final Job job : workflow.getJobs()) {
			for (int i = 0, n = job.getNrInputs(); i < n; i++) {
				...
				Validate.isTrue(input.getSourcePortNr() > -1,
						"Invalid source port number", input.getSourcePortNr());
			}
			for (int i = 0, n = job.getNrOutputs(); i < n; i++) {
				...
				for (final Destination destination : output.getDestinations()) {
					...
					Validate.isTrue(destination.getTargetPortNr() > -1,
							"Invalid target port number",
							destination.getTargetPortNr());
				}
			}
		}
	}
